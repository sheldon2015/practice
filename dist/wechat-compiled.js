...
$(function () {
    ...
    $(city_el).on('click', 'li', function (e) {
        ...
        if (district && district.length != 0) {
            ...
            $(district_el).html(district_list.join("")).show();
        } else if (!district && this.textContent.trim() != '全部') {
            name.forEach(function (el) {
                if (el.provinceId == this.getAttribute('parentId') && el.cityId == this.getAttribute('id')) {
                    el.value = this.getAttribute('data-provinceName');
                    el.districtId = null;
                    flag = true;
                } else if (el.provinceId == this.getAttribute('parentId') && !el.cityId) {
                    el.value = this.getAttribute('data-provinceName');
    ...
    $(district_el).on('click', 'li', function (e) {
        ...
        if (this.textContent.trim() == '全部') {
            ...
            name.forEach(function (el, index) {
                //覆盖三级地区的情况
                if (el.provinceId == this.getAttribute('grandParentId') && el.cityId == this.getAttribute('parentId') && el.districtId == this.getAttribute('id')) {

                    $(this).removeClass('selected');
                    name.splice(index, 1);
                    flag = true;
                    // //覆盖 二级地区的情况
                } else if (el.provinceId == this.getAttribute('grandParentId') && el.cityId == this.getAttribute('parentId') && !el.districtId) {
                        el.value = this.getAttribute('data-districtName');
                        el.districtId = this.getAttribute('id');
                        flag = true;
                        //覆盖一级区域的情况
                    } else if (el.provinceId == this.getAttribute('grandParentId') && !el.cityId && !el.districtId) {
